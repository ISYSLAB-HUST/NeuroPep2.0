...
import com.neuropeptide.entity.RTab;
...
public class SearchReceptorAction extends BaseAction {
    ...
    public String result() throws Exception {
        ...
        sequence = "select t.sequence, t.family, t.organism, t.name, t.modification,t.source,t.accessNum from com.neuropeptide.entity.ReceptorNew t where " + searchStr;
...
//        System.out.println(receptorService.listByEntryName("GASR_BOVIN"));
        ...
        String hql = "from com.neuropeptide.entity.ReceptorNew t where t.accessNum = '" + pepNum + "'";
        NPSearchServiceImpl manager = (NPSearchServiceImpl) this.getBean("searchNeuropeptide");
        new ArrayList();
        ...
        if (list.isEmpty()) {
            ...
            RTab result = new RTab();
            BeanUtils.copyProperties(list.get(0),result);
            //ReceptorNew result = (ReceptorNew) list.get(0);

            result.setTaxa_ID(result.getTaxa_ID());
            result.setTissueSpecifity(result.getTissueSpecifity());
            List<Map<String, Object>> nodes = new ArrayList<>();
            List<Map<String, Object>> lines = new ArrayList<>();
            if (!result.getNeuropeptide().equals("NA")) {

                Map<String, Object> node = new HashMap<>();
                node.put("name", result.getAccessNum());
                node.put("value", 0);
                nodes.add(node);
                String[] arr = result.getNeuropeptide().split("#");
                for (String neuropeptide : arr) {

                    node = new HashMap<>();
                    node.put("name", neuropeptide);
                    node.put("value", 1);
                    nodes.add(node);

                    Map<String, Object> line = new HashMap<>();
                    line.put("source", result.getAccessNum());
                    line.put("traget", neuropeptide);
                    lines.add(line);

//                    hql = "from com.neuropeptide.entity.ReceptorNew t where t.receptor like '%" + neuropeptide + "%'";
//                    List list1 = manager.search(hql);
//                    for (Object o : list1) {
//                        ReceptorNew ReceptorNew = (ReceptorNew) o;
//                        node = new HashMap<>();
//                        node.put("name", ReceptorNew.getAccessNum());
//                        node.put("value", 0);
//                        nodes.add(node);
//
//                        line = new HashMap<>();
//                        line.put("source", ReceptorNew.getAccessNum());
...
//                    }
    ...
    public String basic() throws Exception {
        ...
        String hql = "select t.accessNum, t.sequence, t.length,t.organism,t.family,t.name,t.np_ids from com.neuropeptide.entity.ReceptorNew t where " + searchStr;
        NPSearchServiceImpl manager = (NPSearchServiceImpl) this.getBean("searchNeuropeptide");
        new ArrayList();
    ...
    public String advanced() throws IOException {
        ...
        String hql = "select t.accessNum, t.sequence, t.organism,t.family,t.name, t.modification,t.phyla from com.neuropeptide.entity.ReceptorNew t where " + searchStr;
        NPSearchServiceImpl manager = (NPSearchServiceImpl) this.getBean("searchNeuropeptide");
        new ArrayList();
        List listEntity = manager.search(hql);
        JSONArray jsonArray = JSONArray.fromObject(listEntity);
